...
export default function UploadElement() {
  ...
  const handleUpload = async () => {
    try {
      ...
      const response = await fetch(`/api/cloudbox/upload`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
      ...
      if (allowedTypes.includes(selectedFile.type) && selectedFile.size <= maxFileSize) {
        ...
        let errorMessage = 'Please select a valid file (PDF, JPG, PNG, TXT, JSON, ZIP, GZIP, TAR, RAR, 7Z, MOV, WEBM, FLV, AVI, WAV or MP4, MP3) ';
