...
export default function SideNavbar({}: Props) {
  ...
  return (
    <div
      className={cn(
        "z-[20] relative min-w-[80px] h-full border-r-2 border-muted-forground px-2 py-20 bg-muted/60 backdrop-filter backdrop-blur-lg",
        mobileWidth && "hidden",
      )}
    >
      {!mobileWidth && (
        <div className="absolute right-[-25px] top-13">
          ...
          >
            {isCollapsed ? (
              <ChevronsRight
                size={20}
                className="text-primary dark:hover:text-white"
              />
            ) : (
              <ChevronsLeft
                size={20}
